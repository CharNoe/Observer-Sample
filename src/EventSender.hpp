...
public:
    ...
    auto Disconnect(std::shared_ptr<Event> receiver) -> std::size_t;
    template<class Type>
    auto ConnectQt(Type *receiver) -> sigslot::connection;
    auto DisconnectQt(QObject *receiver) -> std::size_t;

    template<class... FuncArgTypes, class... ArgTypes>
    void operator()(void (Event::*func)(FuncArgTypes...), const ArgTypes &...args)
    {
        auto lambda = [&...args = args, func](Event &event) { (event.*func)(args...); };
...
private:
    ...
    struct EventCallerOf : EventCaller
    {
        ...
        {}
        void Call(Event &event) override { m_lambda(event); }
    ...
    {
        Event *event;
        void operator()(EventCaller &caller) { caller.Call(*event); }
    };
...
template<class Event>
inline auto EventSender<Event>::Connect(std::shared_ptr<Event> receiver) -> sigslot::connection
{
...
template<class Event>
inline auto EventSender<Event>::Disconnect(std::shared_ptr<Event> receiver) -> std::size_t
{
